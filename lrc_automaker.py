...
def generate_lrc(
    ...
    if output_path is None:
        output_path = mp3.with_suffix('.lrc')
    else:
    ...
    try:
        ...
        recognized_path = mp3.parent / (mp3.stem + '_recognized.txt')
        ...
        if lyrics:
            ...
            aligned = align_lyrics(lyrics, result['segments'])
        else:
            aligned = [
                (seg['text'].strip(), seg['start'])
                for seg in result['segments']
                if seg['text'].strip()
            ]
        ...
        merged = []
        for text, ts in aligned:
            if merged and abs(ts - merged[-1][1]) < 0.01:
                merged[-1] = (merged[-1][0] + ' ' + text, ts)
            else:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            for text, ts in merged:
                f.write(timestamp_to_lrc(ts) + text + '\n')
...
def batch_generate_lrc(
    ...
    mp3_files = sorted(src.glob('*.mp3'))
    if not mp3_files:
        logger.warning("目录中没有找到 .mp3 文件: %s", input_dir)
        return []
    ...
    for mp3_path in mp3_files:
        ...
        output_path = out / mp3_path.with_suffix('.lrc').name
        logger.info("处理: %s (歌词: %s)", mp3_path.name, lyrics_path.name if lyrics_path else '无')
        try:
            result = generate_lrc(
                str(mp3_path),
...
def main():
    ...
    parser.add_argument('input', help='输入的 MP3 文件或目录 (配合 --batch)')
    parser.add_argument('lyrics_file', nargs='?', default=None,
                        help='歌词文本文件（每行一句，可选）')
    parser.add_argument('output_file', nargs='?', default=None,
                        help='生成的 LRC 文件路径（可选）')
    ...
    parser.add_argument('--batch', '-b', action='store_true',
                        help='批量处理模式: 将 input 作为目录处理所有 .mp3')
