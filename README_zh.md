...
```bash
python whisper_subtitle.py song.mp3                        # 纯语音识别
python whisper_subtitle.py song.mp3 lyrics.txt              # 带歌词对齐
python whisper_subtitle.py song.mp3 lyrics.txt --language zh  # 中文歌曲
```

### 视频 → SRT 外挂字幕

```bash
python whisper_subtitle.py video.mp4 -f srt                  # 纯语音识别字幕
python whisper_subtitle.py video.mp4 lyrics.txt -f srt       # 带歌词对齐字幕
python whisper_subtitle.py video.mp4 -f srt --separate       # 先人声分离
```

### 进阶用法

```bash
python whisper_subtitle.py video.mp4 -f srt --separate --save-vocals  # 保留人声音轨
python whisper_subtitle.py /path/to/files --batch -f srt              # 批量生成 SRT
python whisper_subtitle.py /path/to/files --batch -f lrc              # 批量生成 LRC
```
...
```
python whisper_subtitle.py <输入> [歌词] [输出] [选项]
```
